...
int16_t robc_ARM6DOF_Anno_ikcalc(float Dest[6])
{
	...
	for (i = 0; i < 8; i++)
	{
		ARM6DOF_para.JointValDest[0] = j1[i / 4];
		ARM6DOF_para.JointValDest[1] = j2[i / 2];
		ARM6DOF_para.JointValDest[2] = j3[i / 2];
		ARM6DOF_para.JointValDest[3] = j4[i];
		ARM6DOF_para.JointValDest[4] = j5[i];
		ARM6DOF_para.JointValDest[5] = j6[i];
 		//if (JointValPossible())
...
int16_t robc_ARM6DOF_Anno_kicalc(float JointVal[6])
{
	...
	double p = 0;
	double y = 0;
	r = atan2(matrix_A6[1][0], matrix_A6[0][0]);
